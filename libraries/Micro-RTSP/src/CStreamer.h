...
public:
    ...
    // CamCar patch: the RTP/JPEG header encodes width/height (as /8), fixed at
    // construction in stock Micro-RTSP. CamCar changes resolution at runtime
    // (auto-adapt), so the streamer updates these per frame before streamFrame()
    // or RTSP clients reconstruct the wrong size.
    void setDimensions(u_short width, u_short height) { m_width = width; m_height = height; }
