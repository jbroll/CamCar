...
**Auth (both stream ports).** :81 and :554 require the **device password** via **Basic auth** — same secret as the port-80 login (`device_pass` in NVS), checked by `PrefEdit::checkBasicAuth` (MJPEG) / the patched `CRtspSession` (RTSP). Username may be **blank or `camcar`**. MJPEG: `MjpegStreamServer::run()` reads the request head on accept and 401s before admitting a client. RTSP: the Micro-RTSP patch 401s `DESCRIBE`/`SETUP`/`PLAY` when unauthenticated (`OPTIONS`/`TEARDOWN` stay open); the streamer's expected password is set from `PrefEdit` per accept, so runtime changes apply to new sessions. Basic is base64, not encryption — fine on the trusted LAN. Clients: MotionEye/NVRs use `http://camcar:<pass>@host:81/stream` or `rtsp://camcar:<pass>@host:554/mjpeg/1`.
